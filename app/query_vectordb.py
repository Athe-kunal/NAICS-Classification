...
class ParentQueryVectorDB(QueryVectorDatabase):
    ...
    def query_parent(
        self, query_embeddings: list[float], parent_num_results: int = 5
        ...
        parent_naics_ids = [
            parent_metadata["NAICS CODE"]
            for parent_metadata in relevant_parent_documents["metadatas"][0]
        ]
...
class ChildQueryVectorDB(QueryVectorDatabase):
    ...
    def query_parent(
        ...
        child_naics_ids = [
            parent_metadata["NAICS CODE"]
            for parent_metadata in relevant_child_documents["metadatas"][0]
        ]
