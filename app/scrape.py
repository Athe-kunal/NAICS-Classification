...
def get_naics_parents_child(driver: webdriver.Chrome):
    ...
    for child_naics in nl_text_list[1:]:
        ...
        if curr_child_industry == curr_loop_industry:
            ...
            naics_dict["child_naics_dict"][-1]["child_description"] += (
                " " + child_naics["industry_description"]
            )
        else:
...
def save_naics(all_naics: list[dict]):
    ...
    with open(config_params["NAICS"]["SAVE_PATH"], "w") as outfile:
...
def scrape_main():
    ...
    element.click()
    time.sleep(1)
    industry_elements = driver.find_elements(
        By.XPATH,
        "//*[contains(@class, 'naics-sector-search-btn') and contains(@class, 'btn-link')]",
    )
